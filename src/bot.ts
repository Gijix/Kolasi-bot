...
class Bot<T extends boolean = boolean> extends Client<T> {
  ...
  async setup () {
    ['commands', 'contextMenuCommands', 'modals', 'componentRows'].forEach((dir) => {
      const dirPath = path.join(process.cwd(),process.env.OUTDIR, dir)
  ...
  async deployCommands (guildId?: string) {
    if (this.applicationCommandsData.length <= 0) return
    try {
